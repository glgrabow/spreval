...
"rotecdf"=function(x,target=NA,ylab=""){
  xecdf<-ecdf(x)#create ecdf object
  fx<-xecdf(x)
  if(ylab==""){ytitle<-nm <-deparse(substitute(x))}else
  {ytitle=ylab}
...
"sfplot"=function(x,target=NA,ylab=""){
  ...
  if(ylab==""){ytitle<-nm <-deparse(substitute(x))}else
  {ytitle=ylab}
  plot(sf,ylim=rev(c(0,max(xx1))),xlim=c(0,1),xlab="cum. frequency",ylab=ytitle, main="")
  abline(h=target,col=2)
  if(target){text(0.2,target,"target depth",srt=0,adj=c(0,1))}
}

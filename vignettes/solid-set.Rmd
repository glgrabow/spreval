...
Note that in the reference, the sprinkler discharge rate (which is a required parameter in `PELQ`)is not given. In this example, the sprinkler discharge rate (38.3 gpm) was simply calculated using the precipitation rate (in./hr) which is the average catch (in.) divided by the irrigation duration (2.25 hr), and the sprinkler and lateral spacing (80 ft x 80 ft). Using the catch data would yield a higher value of PELQ than using actual sprinkler discharge, as drift and evaporation losses from the sprinklers to the catch cans are not accounted for.
...
A plot of an empirical cumulative distribution step function plot that is rotated so that the soil surface is represented at the top of the plot helps to visualize the above results. To do this, the function `spreval::sfplot` may be used.
...
Notice how efficiency increases with increasing target depth, for this example irrigation that applied an average of 5.7 in (catch can average). Efficiency can approach 100% with deficit irrigation, but with greater efficiency comes reduced adequacy as can be seen in the table above.  
